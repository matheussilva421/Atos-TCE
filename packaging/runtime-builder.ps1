...
$licenseReadmePath = Join-Path $PSScriptRoot 'licenses\README.md'
...
# The Mesa is started as ``python -m app.main`` from the package root. The
# embeddable interpreter resolves python314._pth entries against the
# interpreter directory, so the package root (three levels above
# runtime\python) has to be listed for ``import app`` to work; verified with the
# packaged smoke test in packaging/verify-package.ps1.
if (-not ($pthLines -contains '../../..')) { $pthLines = @('../../..') + $pthLines }
