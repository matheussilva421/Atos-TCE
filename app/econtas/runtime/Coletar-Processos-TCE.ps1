...
# Cópia promovida para a Mesa (M6 Task 1). Diferenças em relação ao script do
# pacote portátil:
#   * `-RaizEstado` separa estado de runtime (perfil do navegador e bridge) da
#     árvore de código, para que a Mesa nunca escreva dentro de app/;
#   * a preparação incremental (`-ModoPreparacao progressivo|completo`) continua
#     exigindo a árvore legada, porque a análise agora é do backend. A Mesa roda
#     sempre com `-ModoPreparacao nenhum`;
#   * `-ResultadoJson` grava o recibo por processo (CR-13) que a Mesa usa para
#     decidir se um processo realmente foi baixado. As regiões delimitadas por
#     RECIBO são a única superfície adicional e são removidas pela comparação
#     estática de `tests/test_promoted_equivalence.py`.

$ErrorActionPreference = 'Stop'
$ProgressPreference = 'SilentlyContinue'
[Console]::OutputEncoding = New-Object Text.UTF8Encoding($false)
$scriptRoot = if (-not [string]::IsNullOrWhiteSpace($PSScriptRoot)) { $PSScriptRoot } else { Split-Path -Parent $MyInvocation.MyCommand.Path }
if ([string]::IsNullOrWhiteSpace($RaizEstado)) { $RaizEstado = $scriptRoot }
...
function Start-TceCollectorLease {
    $bridgeRoot = Join-Path $RaizEstado 'dados-locais\bridge'
...
function Test-TceTransferPauseRequested {
    $requestPath = Join-Path $RaizEstado 'dados-locais\bridge\transfer-request.json'
...
try {
    ...
    $receipt = [ordered]@{
        version = 1
        mode = [string]$ModoPreparacao
        lot_number = [int]$NumeroLote
        source_scope = [string]$EscopoPortal
        processes = [ordered]@{}
    }
    #endregion RECIBO
    ...
    for ($index = 0; $index -lt $selected.Count; $index++) {
        ...
        try {
            ...
            if ($result.auth_required -or $result.suspended) {
                ...
                #region RECIBO
                $receipt.processes[$item.key] = [ordered]@{
                    outcome = 'auth_required'
                    downloaded = 0
                    reused = 0
                    deduplicated = 0
                    error = (ConvertTo-TceSafeText $authMessage)
                }
                #endregion RECIBO
            ...
            $receipt.processes[$item.key] = [ordered]@{
                ...
                reused = 0
                deduplicated = 0
                error = (ConvertTo-TceSafeText $_.Exception.Message)
            }
            #endregion RECIBO
    ...
    if (-not [string]::IsNullOrWhiteSpace($ResultadoJson)) {
        $receiptDirectory = Split-Path -Parent $ResultadoJson
        ...
        $receipt | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $ResultadoJson -Encoding utf8
    }
    #endregion RECIBO
