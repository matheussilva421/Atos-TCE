...
Evidências atuais: `docs/notes/2026-09-08-fluxo-portatil-execucao-handoff.md`, commits locais `ca1e372`/`4d0153c`, suítes Python/Node/PowerShell verdes, empacotador oficial e smoke Chrome do ZIP v4 extraído, e `qa_integrated_workflow.py --fixture-only`. A mesa HTML e o painel da extensão agora usam a mesma API `/api/v1/progress/<processo>` quando o serviço está ativo; o modo `file:` permanece fallback local.
