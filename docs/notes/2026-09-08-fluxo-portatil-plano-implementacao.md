...
Evidências atuais: `docs/notes/2026-09-08-fluxo-portatil-execucao-handoff.md`, `docs/notes/2026-09-08-transferencia-quiescente-handoff.md`, commits locais `ca1e372`/`4d0153c`/`a0424ac`, suítes Python/Node/PowerShell verdes, teste de navegador local da mesa, empacotador oficial e smoke Chrome do ZIP v4 extraído, e `qa_integrated_workflow.py --fixture-only`. A mesa HTML e o painel da extensão agora usam a mesma API `/api/v1/progress/<processo>` quando o serviço está ativo; o modo `file:` permanece fallback local. Origin/CSRF e validação server-side de seleção estão endurecidos; colisões de interessado normalizado bloqueiam a exportação. O fechamento de transferência usa pedido cooperativo de pausa, drenagem até 60 s e lease compartilhado.

Não estão comprovados neste checkout: QA funcional no portal real, benchmark dos mesmos 20 processos, instalação/pareamento no Chrome separado, marcação real sem navegação e screenshot de evidência com dados pessoais. O pacote portátil completo foi montado a partir da cópia fornecida pelo usuário, com 227 processos e 4.532 PDFs, e passou auditoria/CRC e `TESTAR-PACOTE.ps1` extraído sem Python/Node no `PATH`; ele representa o snapshot de 05/09/2026, não uma atualização perante o portal. O Chrome separado do projeto foi aberto pelo launcher e aguarda autenticação humana; nenhuma ação de envio/finalização será executada. O teste em segundo PC foi dispensado explicitamente pelo usuário e não é requisito desta entrega. Os demais gates dependem de acesso humano/ambiente externo e permanecem explicitamente como candidato, não release validada. Caminhos abreviados devem ser expandidos pelas convenções do cabeçalho. Release/version pin de PDF.js é um gate explícito da fase 4, dependente do navegador alvo, não autorização para buscar dependências flutuantes durante execução normal.
