...
- [x] Gate: datas/matrículas/cargo com classe preservados; PDF 1 página, escaneado, rotação 90/180/270, evento repetido, candidato múltiplo, hash modificado e round-trip de checkpoint. Validado por 58 testes focados, teste OCR real em PDF escaneado de uma página e suíte Python completa; a transformação nativa foi corrigida para CropBox/rotação e o TSV portátil passou a usar `tessedit_create_tsv=1`.
...
- [x] Gate visual 900x1440 e desktop: divisor, zoom, páginas completas, fonte legível, ausência de overflow horizontal raiz. Scroll interno de PDF permitido; não prometer todos os documentos na tela simultaneamente. Validado no HTML do ZIP v5 extraído, com screenshot 900x1440 e controles de zoom/rotação/reset.
...
- [x] Extrair ZIP em pasta nova com espaços/acentos. Iniciar sem Python/Node no PATH; runtime interno, manifest, schema, auditoria, ordem e progresso verificados. Instalação unpacked/pareamento no Chrome separado continuam no gate de QA supervisionado.
...
- [x] QA em Chrome separado: usuário autenticou; o smoke de fixture executou 5 testes em perfil descartável e o Chrome do projeto executou o preenchimento no formulário real autorizado. Foram preenchidos somente os campos disponíveis, sem envio/finalização ou navegação; os sete campos foram restaurados aos valores originais vazios ao final.
- [x] Verificar HTML noutra janela acompanhando processo/interessado em até 2s, pause/resume, dois PDFs em abas, zoom/rotação, todos os sete links de evidência e marca Concluído sem navegar. A execução fixture-only no HTML do ZIP v5 confirmou os itens de mesa; seis fontes resolveram PDF/página e o sétimo campo (gênero) permaneceu explicitamente pendente/sem evidência.
...
- [x] Testar serviço bloqueado e fallback, retomada após 401/403 e pareamento funcional no pacote extraído. `qa_extracted_service.py` confirmou 401, 403/CSRF, seleção, descarte de sequência antiga, bloqueio por transferência e fallback de porta. O gate de segundo PC foi dispensado explicitamente pelo usuário.
...
- [x] Commit final, verificar remoto, push somente se houver destino autorizado e sem dados privados. Não criar/publicar remoto automaticamente. Commit local desta continuação `af95f9c`; sem `origin`, portanto sem push.
...
Plano elaborado em 08/09/2026 a partir dos fontes existentes. A execução posterior implementou as fases 1–7 como candidato integrado e adicionou QA fixture-only e QA manual supervisionado na fase 8. Em 08/09/2026 foi acrescentado QA no Chrome separado do projeto, com usuário autenticado: a extensão foi instalada a partir do pacote extraído, a pesquisa por processo/nome foi exercitada, o sinal tipado foi capturado no frame real e o preenchimento autorizado foi exercitado e revertido sem envio/finalização. A extensão continua sendo o único componente autorizado a escrever nos sete campos; o botão de Complementar Ato apenas envia o sinal tipado, e a pesquisa manual não altera a identidade do portal.

Evidências atuais: `docs/notes/2026-09-08-fluxo-portatil-execucao-handoff.md`, `docs/notes/2026-09-08-transferencia-quiescente-handoff.md`, `docs/notes/2026-09-08-benchmark-20-handoff.md`, commits locais `9cf9af5`/`e2d2016`/`af95f9c`, suítes Python/Node/PowerShell verdes, smoke de navegador da extensão, teste da mesa HTML, empacotador oficial, pacote v6 extraído sem Python/Node no `PATH`, `qa_html_gates.py`, `qa_real_geometry.py`, `qa_extracted_service.py` e QA manual do Chrome real. A mesa HTML e o painel da extensão agora usam a mesma API `/api/v1/progress/<processo>` quando o serviço está ativo; o modo `file:` permanece fallback local. Origin/CSRF e validação server-side de seleção estão endurecidos; colisões de interessado normalizado bloqueiam a exportação. O fechamento de transferência usa pedido cooperativo de pausa, drenagem até 60 s e lease compartilhado. O empacotador regenera HTML privado stale dentro do staging e os assets versionados de `portable/app/web` incluem controles PDF.js locais.

Permanece não comprovado neste checkout somente o benchmark dos mesmos 20 processos com baseline humano, mediana/p95 de sincronização e cliques, além da meta quantitativa de p95 <=2s. A fixture, o pacote extraído, o serviço bloqueado/fallback, a retomada após 401/403, a geometria OCR/rotação/hash/checkpoint e o preenchimento supervisionado no portal real foram comprovados nos limites documentados. O preenchimento real foi restaurado para os valores vazios originais; nenhuma ação de envio/finalização foi executada. O pacote portátil completo foi montado a partir da cópia fornecida pelo usuário, com 227 processos e 4.532 PDFs, e representa o snapshot de 05/09/2026, não uma atualização perante o portal. O teste em segundo PC foi dispensado explicitamente pelo usuário e não é requisito desta entrega. Enquanto o benchmark não tiver os mesmos 20 processos e observações humanas, o resultado deve ser tratado como candidato integrado, não release portátil validada. Caminhos abreviados devem ser expandidos pelas convenções do cabeçalho. Release/version pin de PDF.js é um gate explícito da fase 4, dependente do navegador alvo, não autorização para buscar dependências flutuantes durante execução normal.
