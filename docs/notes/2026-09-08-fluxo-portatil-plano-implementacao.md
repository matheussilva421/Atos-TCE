...
- [ ] Repetir QA sobre ZIP extraído, não apenas source tree. Testar serviço bloqueado e fallback. O gate de segundo PC foi dispensado explicitamente pelo usuário.
...
Evidências atuais: `docs/notes/2026-09-08-fluxo-portatil-execucao-handoff.md`, `docs/notes/2026-09-08-transferencia-quiescente-handoff.md`, commits locais `42fef65`/`141db71`, suítes Python/Node/PowerShell verdes, teste de navegador local da mesa, empacotador oficial, smoke Chrome do ZIP v4 extraído, `qa_integrated_workflow.py --fixture-only` e QA manual parcial do Chrome real. A mesa HTML e o painel da extensão agora usam a mesma API `/api/v1/progress/<processo>` quando o serviço está ativo; o modo `file:` permanece fallback local. Origin/CSRF e validação server-side de seleção estão endurecidos; colisões de interessado normalizado bloqueiam a exportação. O fechamento de transferência usa pedido cooperativo de pausa, drenagem até 60 s e lease compartilhado.

Não estão comprovados neste checkout: preenchimento em fixture como gate integrado da entrega, os demais gates do portal real (HTML em outra janela, dois PDFs em abas, zoom/rotação, sete links, `Concluído` sem navegação), benchmark dos mesmos 20 processos, pareamento com a mesa/serviço e retomada em ambiente restrito. A instalação unpacked no Chrome separado, pesquisa e sinal sem envio estão comprovados no handoff, mas não equivalem a release validada. O pacote portátil completo foi montado a partir da cópia fornecida pelo usuário, com 227 processos e 4.532 PDFs, e passou auditoria/CRC e `TESTAR-PACOTE.ps1` extraído sem Python/Node no `PATH`; ele representa o snapshot de 05/09/2026, não uma atualização perante o portal. Nenhuma ação de envio/finalização foi executada. O teste em segundo PC foi dispensado explicitamente pelo usuário e não é requisito desta entrega. Os demais gates dependem de acesso humano/ambiente externo e permanecem explicitamente como candidato, não release validada. Caminhos abreviados devem ser expandidos pelas convenções do cabeçalho. Release/version pin de PDF.js é um gate explícito da fase 4, dependente do navegador alvo, não autorização para buscar dependências flutuantes durante execução normal.
