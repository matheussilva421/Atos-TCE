...
Plano elaborado em 08/09/2026 a partir dos fontes existentes. A execução posterior implementou as fases 1–7 como candidato integrado e adicionou QA fixture-only na fase 8. Em 08/09/2026 foi acrescentado QA manual parcial no Chrome separado do projeto, com usuário autenticado: a extensão foi instalada a partir do pacote extraído, a pesquisa por processo/nome foi exercitada e o sinal tipado foi capturado no frame real sem alterar campos documentais. A extensão continua sendo o único componente autorizado a escrever nos sete campos; o botão de Complementar Ato apenas envia o sinal tipado, e a pesquisa manual não altera a identidade do portal.

Evidências atuais: `docs/notes/2026-09-08-fluxo-portatil-execucao-handoff.md`, `docs/notes/2026-09-08-transferencia-quiescente-handoff.md`, commits locais `42fef65`/`141db71`, suítes Python/Node/PowerShell verdes, teste de navegador local da mesa, empacotador oficial, pacote v5 extraído sem Python/Node no `PATH`, `qa_html_gates.py --html <v5-extraído>`, `qa_integrated_workflow.py --fixture-only` e QA manual parcial do Chrome real. A mesa HTML e o painel da extensão agora usam a mesma API `/api/v1/progress/<processo>` quando o serviço está ativo; o modo `file:` permanece fallback local. Origin/CSRF e validação server-side de seleção estão endurecidos; colisões de interessado normalizado bloqueiam a exportação. O fechamento de transferência usa pedido cooperativo de pausa, drenagem até 60 s e lease compartilhado. O empacotador regenera HTML privado stale dentro do staging e os assets versionados de `portable/app/web` incluem controles PDF.js locais.

Não estão comprovados neste checkout: preenchimento em fixture como gate integrado da entrega, os demais gates do portal real, benchmark dos mesmos 20 processos, gate geométrico completo para OCR/rotação/hash/checkpoint, pareamento funcional com a mesa/serviço em pacote extraído, serviço bloqueado/fallback e retomada em ambiente restrito. A instalação unpacked no Chrome separado, pesquisa e sinal sem envio estão comprovados no handoff, mas não equivalem a release validada. O HTML do ZIP v5 extraído passou a validação visual/funcional de mesa, evidências, duas abas, zoom/rotação e `Concluído` sem navegação; isso não substitui o gate completo do portal nem a medição comparativa. O pacote portátil completo foi montado a partir da cópia fornecida pelo usuário, com 227 processos e 4.532 PDFs, e passou auditoria/CRC e `TESTAR-PACOTE.ps1` extraído sem Python/Node no `PATH`; ele representa o snapshot de 05/09/2026, não uma atualização perante o portal. Nenhuma ação de envio/finalização foi executada. O teste em segundo PC foi dispensado explicitamente pelo usuário e não é requisito desta entrega. Os demais gates dependem de acesso humano/ambiente externo e permanecem explicitamente como candidato, não release validada. Caminhos abreviados devem ser expandidos pelas convenções do cabeçalho. Release/version pin de PDF.js é um gate explícito da fase 4, dependente do navegador alvo, não autorização para buscar dependências flutuantes durante execução normal.
