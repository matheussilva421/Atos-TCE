...
- Captura L0 por Playwright sobre todos os frames do portal: 1 página, 9 documentos, 41 controles estruturais. O inventário atual contém frames duplicados `iframeOBJ`, com `/Home.asp` e `/SISTEMAS/Processo/expirou.asp`; nenhum frame está em `ProcessonoSetor.asp` e nenhum destino de paginação foi encontrado. O estado exato do frame de lista é, portanto, ambíguo; não houve transição L1/L2.
