...
function Get-TcePortableNodeStatus {
    ...
    foreach ($relative in @('INICIAR.cmd', 'Coletar-Processos-TCE.ps1', 'app\menu.ps1', 'app\analysis_pipeline.py', 'app\evidence_geometry.py', 'app\bridge_auth.py', 'app\local_service.py', 'app\qualification.py', 'app\workflow_state.py', 'app\filter_new_batch.py', 'app\extension_exporter.py', 'app\package_complete_archive.py')) {
        $path = Join-Path $root $relative
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { continue }
