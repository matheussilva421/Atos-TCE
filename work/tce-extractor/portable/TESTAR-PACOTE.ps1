...
function Get-TcePortableExtensionManifestStatus {
    ...
    if (($permissions -join '|') -ne 'storage|sidePanel') { Add-TcePortableError $errors 'permissoes devem ser exatamente storage e sidePanel' }
    if (($hostPermissions -join '|') -ne 'https://novaarearestrita.tce.rn.gov.br/*') { Add-TcePortableError $errors 'host permitido deve ser exatamente https://novaarearestrita.tce.rn.gov.br/*' }
...
function Get-TcePortableNodeStatus {
    ...
    foreach ($relative in @('INICIAR.cmd', 'Coletar-Processos-TCE.ps1', 'app\menu.ps1', 'app\analysis_pipeline.py', 'app\extension_exporter.py', 'app\package_complete_archive.py')) {
        $path = Join-Path $root $relative
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { continue }
