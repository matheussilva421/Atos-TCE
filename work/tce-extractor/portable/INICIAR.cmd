...
rem INICIAR.cmd envio-real inicia a ponte com o envio automatico de atos habilitado.
if /I "%~1"=="parar" (
...
) else if /I "%~1"=="envio-real" (
    powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "$utf8 = New-Object System.Text.UTF8Encoding($false); [Console]::InputEncoding = $utf8; [Console]::OutputEncoding = $utf8; $OutputEncoding = $utf8; $menuPath = Join-Path $env:TCE_PORTABLE_PACKAGE_ROOT 'app\menu.ps1'; $menu = [ScriptBlock]::Create([IO.File]::ReadAllText($menuPath, $utf8)); & $menu -MenuRoot (Join-Path $env:TCE_PORTABLE_PACKAGE_ROOT 'app') -LaunchLocalService -EnableRealSend"
) else if /I "%~1"=="abrir-mesa" (
    powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "$utf8 = New-Object System.Text.UTF8Encoding($false); [Console]::InputEncoding = $utf8; [Console]::OutputEncoding = $utf8; $OutputEncoding = $utf8; $menuPath = Join-Path $env:TCE_PORTABLE_PACKAGE_ROOT 'app\menu.ps1'; $menu = [ScriptBlock]::Create([IO.File]::ReadAllText($menuPath, $utf8)); & $menu -MenuRoot (Join-Path $env:TCE_PORTABLE_PACKAGE_ROOT 'app') -OpenReview"
) else if /I "%~1"=="ponte" (
