...
def _refresh_staged_review_html(staging_root: Path, distribution: str) -> bool:
    """Regenerate the offline desk in the package staging tree.

    A portable source can contain an HTML snapshot generated by an older
    pipeline revision.  The current generator and its source metadata are
    already part of a complete private package, so refresh only the staged
    copy and never mutate the user's source tree.  Small audit fixtures that
    intentionally omit the extraction metadata keep their existing HTML.
    """

    if distribution != "private":
        return False
    archive = Path(staging_root) / "acervo-tce"
    generator_path = Path(staging_root) / "app" / "html_generator.py"
    manifest_path = archive / "pdfs-alvo-manifest.json"
    checkpoint_path = archive / "checkpoint-extracao.json"
    archive_index_path = archive / "indice-classificado.json"
    output_path = archive / "complementar-ato.html"
    required = (
        generator_path,
        manifest_path,
        checkpoint_path,
    ...
    spec = importlib.util.spec_from_file_location(
        "_tce_staged_html_generator", generator_path
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"gerador HTML portátil inválido: {generator_path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    writer = getattr(module, "write_html", None)
...
def build_complete_zip(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="tce-package-staging-") as staging_text:
        ...
        _refresh_staged_review_html(staging_root, distribution)
